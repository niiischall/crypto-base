...
/*!*****************************************!*\
  !*** ./src/store/actions/actionHome.ts ***!
  \*****************************************/
/*! exports provided: getPopularCoins, getPopularCoinsDetails, followCoin, unfollowCoin, getFollowingCoinsDetails, startNotifications, stopNotifications, getNotifiedCoinsDetails */
...
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "getNotifiedCoinsDetails", function() { return getNotifiedCoinsDetails; });
...
const getNotifiedCoinsDetailsSuccess = coinsInfo => {
  return {
    type: _actionTypes__WEBPACK_IMPORTED_MODULE_0__["FETCH_NOTIFIED_COINS_DETAILS_SUCCESS"],
    coinsInfo
  };
};

const getNotifiedCoinsDetailsFailure = error => {
  return {
    type: _actionTypes__WEBPACK_IMPORTED_MODULE_0__["FETCH_FOLLOWING_COINS_DETAILS_FAILURE"],
    error
  };
...
const getNotifiedCoinsDetails = coins => {
  return async dispatch => {
    const data = coins.map(coin => coin.slug);
    const requestURL = `${_services_api__WEBPACK_IMPORTED_MODULE_1__["apiEndPoints"].coinsInfo}${data.join(',')}`;
    ...
    try {
      ...
      if (response.ok) {
        const json = await response.json();
        dispatch(getNotifiedCoinsDetailsSuccess(json.data));
      } else {
        dispatch(getNotifiedCoinsDetailsFailure('Unable to fetch notified coins details.'));
      }
    } catch (error) {
      dispatch(getNotifiedCoinsDetailsFailure('Unable to fetch notified coins details.'));
    }
...
//# sourceMappingURL=app.58b42b62be296879a6a9.hot-update.js.map