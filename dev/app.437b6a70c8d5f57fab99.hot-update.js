...
const SearchInput = props => {
  ...
  const searching = () => {
    if (totalCoins.length > 0) {
      const filteredSearches = totalCoins.filter((coin, index) => {
        if (coin.toLowerCase().includes(search.toLowerCase()) && index <= 100) {
          return coin;
        }
      });
...
//# sourceMappingURL=app.437b6a70c8d5f57fab99.hot-update.js.map